 ...
 *
 * 검증은 각 프롬프트의 "완료 정의(DoD)"로 한다(별도 통합 스크립트 만들지 않음 — 프롬프트와 싱크 유지).
...
export const meta = {
  name: 'build-backend',
  description: '백엔드 빌드 프롬프트(be/01~07)를 파운데이션 → 기능 병렬로 오케스트레이션',
  phases: [
...
function runPrompt(file) {
  return [
    ...
    '그 프롬프트가 참조하라는 docs(backend-structure.md, contracts/data-models.md, features/<도메인>/api.md, MVP-SCOPE.md, prompts/*.md)를 단일 소스로 삼는다.',
...
log('✅ 백엔드 빌드 하니스 완료 (검증은 각 프롬프트의 DoD로)')
