...
| `healing-prompt.md` | f4-jobs (자가치유) | 실패 `.out` 진단 → `.inp` 경로형 FIX 생성. **`be/05`가 사용** |
...
> 백지 MVP에서 **f2(플랜)·f5(리포트)·f4(자가치유)** 가 이 프롬프트로 실제 LLM을 호출한다(= Claude 3대 호출 지점, 데모 하이라이트). 키가 없으면 빌드 프롬프트가 목/스킵 폴백.
