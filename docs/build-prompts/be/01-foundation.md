...
5. **공유 유틸/엔진**: `app/shared/jobs_mock.py`(f4 **폴백용**(`USE_SGE=0`/SSH 실패 시) 가짜 스트림). **`schema_engine`·`self_healing`·`physics_rules`·inp 로직(`options`/`physics_patterns`)은 f3(be/04)·f4(be/05)가 그 md 명세대로 재구현**한다(코드 복사 아님). be/01은 폴더 골격만 두고, **데이터 파일(`cp2k_input.xml`·`.cache.pkl`·`basis_map.json`·`healing_knowledge.json`)을 `app/shared/`에 둔다**(반입). 실제 SGE 제출은 `app/core/sge.py` SSH.
6. **실행 설정**: `backend/`에서 `uvicorn app.main:app --reload --port 8000`로 기동. `.env.example`에 `CLAUDE_API_KEY=` + 클러스터 변수(`USE_SGE`/`CLUSTER_HOST`/`CLUSTER_PORT`/`CLUSTER_USER`/`CLUSTER_PASSWORD`/`CLUSTER_REMOTE_ROOT`/`CLUSTER_QUEUE`/`CLUSTER_PE`/`CLUSTER_MPI_RANKS`/`CP2K_ROOT`/`CP2K_DATA_DIR`/`CP2K_MPIEXEC`/`CP2K_SETVARS`/**`CP2K_VENV`**)를 **키만(값 비움)** 나열. (**`CP2K_VENV`** = run.sh가 `source`할 클러스터 가상환경 activate 경로 — be/05 `SGE_TEMPLATE` 필수. 빠지면 qsub 잡이 즉시 죽는다.) `requirements.txt`(fastapi, uvicorn, anthropic, ase, python-dotenv, pydantic, **paramiko**, **lxml**(schema_engine XML 파싱)).
