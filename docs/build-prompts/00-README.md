...
- `docs/design-system.md` — 토큰·레이아웃·컴포넌트·CSS 함정(§4.5)·원본 델타(§8)
...
- `docs/mockups/a-paper-hifi.html` — 픽셀 단위 시각 레퍼런스

## 실행 순서 (자세한 분업·타임라인은 `WORKPLAN.md`)
1. **파운데이션 먼저(FE·BE 병행 가능)**: `be-01-foundation.md`(백엔드 스캐폴드) + `01-foundation.md`(프런트 스캐폴드). **반드시 먼저.**
2. **기능별(한 사람 = 한 기능 풀스택)**: 자기 기능의 **`be-0X`(백엔드) + `0X`(프런트)** 를 함께 실행. f1~f6. 여러 명이면 병렬, 혼자면 f1→f6 순서.
