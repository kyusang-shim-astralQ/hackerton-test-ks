...
> 사용법: `fe/01-foundation.md` 완료 후, 새/같은 세션(프로젝트 루트)에 아래 프롬프트를 붙여넣으세요.
...
- `docs/design-system.md` §4.2(2·3단계 화면: 전자 구조 설정 | SCF 수렴 설정 카드, 그 아래 전체 폭 AI 계산 플랜).

### 만들 것
1. **2단계 (`step-2`) 물성 선택**: 카테고리(정적/동역학·열적/화학반응성/광학/전자·전하/기타)로 묶인 **12개 물성 중 단일 선택(라디오)** — 한 번에 하나의 target property만 고를 수 있다. 선택을 wizard-store에 저장. 광학(absorption/emission) 선택 시 "TDDFPT → DIAGONALIZATION 고정" 안내.
2. **3단계 (`step-3`) DFT 옵션**: 두 설정 카드를 **동일 높이로 나란히**(design-system §4.5 — `height` 금지·stretch만, 그리드 카드 `margin-top:0`):
   - 전자 구조 설정: 범함수/기저/유사퍼텐셜/평면파·상대 컷오프/스핀(RKS·UKS).
...
### 완료 정의 (DoD)
- [ ] 2단계 물성 단일 선택(라디오, 12개 중 하나)이 store에 반영, 광학 안내 동작.
