...
| **f4 제출/모니터** | ✅ REAL (SSH/SGE) | `paramiko`로 클러스터(`CLUSTER_*`) 접속 → `.inp`+`run.sh` 업로드 → 실제 `qsub` → `qstat` 폴링 → 결과 회수(`app/core/sge.py`). `USE_SGE=0`/연결 실패 시 목 스트림 폴백. |
...
완전 스키마 검증, `self_healing` 지식베이스(자가치유 루프), `physics_rules` 전체. (f4 **실제 SGE 제출은 범위 안** — `core/sge.py` SSH로 처리.)
→ reference 코드가 허용되면 자가치유·스키마검증을 별도로 이식해 강화하면 됨(별도 프롬프트 세트).
