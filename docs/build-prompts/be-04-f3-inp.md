...
- **shared/inp_template.py**: `PlanStep` + atom_info + params로 **유효한 CP2K `.inp` 텍스트**를 만드는 템플릿 함수. 최소 유효 블록: `&GLOBAL`(PROJECT/RUN_TYPE), `&FORCE_EVAL`(METHOD QS, `&DFT`(BASIS/POTENTIAL, `&MGRID` CUTOFF, `&XC` FUNCTIONAL, `&SCF` EPS_SCF/MAX_SCF, 필요시 `&KPOINTS`)), `&SUBSYS`(`&CELL` ABC/ANGLES, `&COORD`, `&KIND`). RUN_TYPE에 따라 `&MOTION` 등 분기.
- **service.py**: `generate_inp_logic` — selected/exclude 필터 후 1-based 재인덱싱 → 스텝별 `inp_template`로 `.inp` 생성. 단일/다중(`multi_atom_info`) 분기·파일명(`step{i}.inp` / `{base}_step{i}.inp`)은 api.md대로. k-point 우선순위도 api.md(3단계)대로.
